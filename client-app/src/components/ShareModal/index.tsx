...
const ShareModal = ({ url, isOpen, onClose }: { url: string, isOpen: boolean, onClose: () => void }) => {
  ...
  const socialMedia = [
    { name: 'Facebook', svg: FacebookSVG, shareUrl: `https://www.facebook.com/sharer/sharer.php?u=${url}` },
    { name: 'WhatsApp', svg: WhatsAppSVG, shareUrl: `https://api.whatsapp.com/send?text=${url}` },
    { name: 'Email', svg: EmailSVG, shareUrl: `mailto:?body=${url}` },
    { name: 'Reddit', svg: RedditSVG, shareUrl: `https://reddit.com/submit?url=${url}` },
      ...
      <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white " onClick={(e) => e.stopPropagation()}>
          ...
          <div className=" flex justify-center items-center mt-2 px-7 py-3">
            {socialMedia.map((media) => (
              <Button key={media.name} onClick={() => handleSocialShare(media.shareUrl)} variant="text">
                <img src={media.svg} alt={`${media.name} icon`} className="w-12 h-12" />
              </Button>
            ))}
          </div>
